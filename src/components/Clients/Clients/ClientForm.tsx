...
export default function ClientForm({ mode, isEdit, control, errors }: Props) {
    ...
    return currencyData
      ? currencyData?.map((account: BusinessCurrency) => {
          return {
            ...
            value: account?.businesscurrencyId
              ? account?.businesscurrencyId
              : 0,
  ...
  const bankAccountDataList = (id: number) => {
    return bankData
      ? bankData?.map((account: BankAccount) => {
          return {
            text: account?.bankAccountTitle ? account?.bankAccountTitle : "",
            value: account?.bankAccountId ? account?.bankAccountId : 0,
            defaultSelected: account?.bankAccountId === id,
          };
        })
      : [];
  };
      ...
      <div className="row">
        <div className="col-lg-4 col-md-5 col-11 my-2">
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <MDBSelect
                inputClassName={`${errors.payableAccountId && "is-invalid"}`}
          ...
          />
          <FormValidationError errorMessage={errors.payableAccountId?.message} />
        </div>
        ...
        <div className="col-lg-4 col-md-5 col-11 my-2">
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <MDBSelect
                label="Sales Account"
                inputClassName={errors.salesAccountId && "is-invalid"}
                data={payableAccDataList(value ?? 0)}
                onValueChange={(data) => {
                  if ("value" in data) {
                    onChange(data.value);
                  }
          ...
          />
          <FormValidationError errorMessage={errors.salesAccountId?.message} />
        </div>
      </div>
      <div className="row">
        <div className="col-lg-4 col-md-5 col-11 my-2">
          <Controller
            ...
            render={({ field: { onChange, value } }) => (
              <MDBSelect
                data={bankAccountDataList(value ?? 0)}
                onValueChange={(data) => {
                  if ("value" in data) {
                    onChange(data.value);
                  }
