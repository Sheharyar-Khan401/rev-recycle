...
import { useLazyGetClearingInvoicesReportsQuery } from "redux/features/purchase/Invoices/InvoicesApiSlice";
...
export default function ClearingInvoices() {
  const ref = useRef<HTMLInputElement | null>(null);
  const [triggerClearingInvoicesReports, getClearingInvoicesReports] =
  ...
  useEffect(() => {
    triggerClearingInvoicesReports(queryParams);
  }, [queryParams]);

  const rows = getClearingInvoicesReports?.data
    ? getClearingInvoicesReports?.data?.payLoad?.map(
  ...
  return (
    <div className="d-lg-flex">
      <div>
        <PurchaseReportsSideNav type={13} />
      </div>
      <div className="table-container">
        ...
        <DataTable
          ref={ref}
          tableTitle="Clearing Invoices Reports"
          isLoading={getClearingInvoicesReports.isFetching}
          columns={[
