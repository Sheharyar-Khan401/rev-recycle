...
import { useLazyGetVoucherQuery } from "redux/features/finance/primarydata/bankreceiptvoucherApiSlice";
...
export default function CashPaymentReceiptVouchers() {
  ...
  const cashPaymentVoucherData = result?.data?.payLoad;
  ...
  return (
    <div className="d-lg-flex">
      <VouchersSideNav type={1} />
      <div className="table-container">
        <Filters
          componentRef={ref}
          addRedirectPath={hasPermission("fin_v_100") ? "add" : ""}
          printAble={
            (cashPaymentVoucherData && cashPaymentVoucherData.length > 0) ??
            false
          }
          exportAble={
            (cashPaymentVoucherData && cashPaymentVoucherData.length > 0) ??
        ...
        {hasPermission("fin_v_103") && (
          <DataTable
            ref={ref}
            tableTitle="Cash Payment Vouchers"
            ...
            rows={
              cashPaymentVoucherData
                ? cashPaymentVoucherData?.map((item: Voucher) => {
                    // Calculate the total debitAmount and creditAmount for the list of vouchers
                    ...
                    return {
                      ...
                      voucherExchangeRate: item?.voucherExchangeRate
                        ...
                        : "-",
                      debitAmount: debit ? debit : 0, // Set the total debitAmount
                      creditAmount: credit ? credit : 0, // Set the total creditAmount
